...
class ModelCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		//$this->deleteCascade= $this->generateDeleteCascade();
		foreach($tables as $table)
		{
			...
			$params=array(
				...
				//'deleteCascade'=>isset($this->deleteCascade[$className]) ? $this->deleteCascade[$className] : array(),
			);
	...
	public function generateRules($table)
	{
		...
		//esto es mio y me la pelan
		$dropdownlist = array();
		$autocompleatelist =array();
		$datelist = array();
		...
		foreach($table->columns as $column)
		{
			...
			//else if($column->type==='date' || $column->type==='datetime' || $column->type==='timestamp')
				//$datelist[]=$column->name;
			else if(!$column->isPrimaryKey && !$r)
				$safe[]=$column->name;
			
			if(strcasecmp(substr($column->name,-4),'_did')===0 )
				$dropdownlist[]=$column->name;
			else if(strcasecmp(substr($column->name,-3),'_id')===0)
				$dropdownlist[]=$column->name;
			else if(strcasecmp(substr($column->name,-4),'_aid')===0)
				$autocompleatelist[]=$column->name;
		}
		if($required!==array())
		...
		if($dropdownlist!==array())
			$rules[]="array('".implode(', ',$dropdownlist)."','dropdownfield')";
		if($autocompleatelist!==array())
			$rules[]="array('".implode(', ',$autocompleatelist)."','autocompletefield')";
		if($datelist!==array())
			$rules[]="array('".implode(', ',$autocompleatelist)."','datefield')";
	...
	/*protected function generateDeleteCascade()
	{
		if(!$this->buildRelations)
			return array();
		$relations=array();
